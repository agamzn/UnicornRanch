...
class UnicornRanch(object):
    ...
    def lab2(self) -> dict:
        ...
        # Get the lab1 data and prep a dict
        initial_tax_data = self.get_taxes()
        answer = {}

        # Triple rate, then reduce by 10%
        answer['total_tax'] = initial_tax_data['total_tax'] * 3 * 0.9

        # Calculate the new effective rate per acre
        answer['effective_rate'] = answer['total_tax'] / ((self.ranch_sqft + self.barn_sqft) / self.SQUARE_FEET_PER_ACRE)
